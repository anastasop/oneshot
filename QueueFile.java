...
public class QueueFile {
  ...
  /** Position of primary header. The status byte is in position 0. */
  private static final int PRIMARY_HEADER = 1;
 
  /** Length of primary or backup header in bytes. */
  private static final int PARTIAL_HEADER_LENGTH = 16;
 
  /** Position of backup header. */
  private static final int BACKUP_HEADER = PRIMARY_HEADER
      + PARTIAL_HEADER_LENGTH;
 
  /** Total length of header in bytes (includes state and scratch header). */
  static final int HEADER_LENGTH = 1 + PARTIAL_HEADER_LENGTH * 2;
 
  /** Header state values. */
  interface HeaderState {
    /** The primary header is in a good state. */
    byte COMMITTED = 0;
 
    /**
     * We're in the middle of updating the primary header. The backup header
     * is in a good state.
     */
    byte UPDATING = 1;
  }
  ...
  /**
   * The underlying file. Uses a ring buffer to store entries. We back up the
   * header before modifying it so we can recover from crashes. Storing the
   * file length ensures we can recover from a failed expansion (i.e. if
   * setting the file length succeeds but the process dies before the data can
   * be copied).
   *
   * <pre>
   *   Format:
  ...
  public QueueFile(File file) throws IOException {
    ...
    // If the header wasn't fully written, roll back the change.
    raf.seek(0);
    int headerState = raf.read();
    if (headerState != HeaderState.COMMITTED) recoverHeader();
  ...
  /**
   * Copies the backup header to the main header. Effectively rolls back
   * an earlier transaction.
   */
  void recoverHeader() throws IOException {
    // Copy from backup to primary.
    FileChannel channel = raf.getChannel();
    channel.position(PRIMARY_HEADER);
    if (channel.transferTo(BACKUP_HEADER, PARTIAL_HEADER_LENGTH, channel)
        != PARTIAL_HEADER_LENGTH) {
    ...
    // Update status.
    raf.seek(0);
    raf.write(HeaderState.COMMITTED);
  }
 
  /**
   * True if we're updating the main header. Indicates that we should
   * restore the header from its backup before making further modifications.
   * If we allowed both headers to be corrupted, the file is corrupt.
   */
  private boolean updating;
 
  /**
   * Writes header atomically. The arguments contain the updated values. The
   * class member fields should not have changed yet (they're used for
   * the backup). This only updates the state in the file. It's up to the
   * caller to update the class member variables *after* this call succeeds.
   */
  private void writeHeader(int fileLength, int elementCount, int firstPosition,
      int lastPosition) throws IOException {
    /*
     * If a previous header update failed, we *must* restore the original header
     * before proceeding. We can't risk corrupting both the primary and
     * backup headers. This effectively rolls back the previous failed change.
     */
    if (updating) {
      recoverHeader();
      updating = false;
    ...
    writeInts(buffer, this.fileLength, this.elementCount, this.first.position,
        this.last.position);
    raf.seek(BACKUP_HEADER);
    raf.write(buffer);
    ...
    updating = true;
    raf.seek(0);
    raf.write(HeaderState.UPDATING);
    raf.write(buffer);
  ...
  public synchronized void add(byte[] data, int offset, int count)
    ...
    int position = isEmpty() ? HEADER_LENGTH
    ...
    // Commit the addition.
    writeHeader(fileLength, elementCount + 1, first.position, newLast.position);
    last = newLast;
    elementCount++;
    if (first == Element.NULL) first = last; // first element
  }
